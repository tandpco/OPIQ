...
$(window).on('scroll', function() {
	...
	if (dist >= 42) {
		$('.report__header').addClass('opaque');
	} else {
		$('.report__header').removeClass('opaque');
	}
});
...
$('.dl-overlay').on('click', function(e) {
	if (e.target == $('.dl-overlay')[0] || e.target == $('.text')[0])
		$('body').removeClass('download');
});

function generatePDF() {
	var report = $('#reportWrapper').html();
	$('form')
		.append(
			$('<input>', {type: 'hidden', name: 'html', value: report})
		)
		.submit();
}
...
// 	$('form')
// 		.append($("<input>", {type : 'hidden', name : 'analysis', value : $('.analysis').text()}))
